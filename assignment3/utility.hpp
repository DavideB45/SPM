...
// 'dname' is a directory; traverse it and call doWork() for each file
// returns false in case of error
static inline void walkDirPar(const std::string &dname, const bool comp, std::atomic<bool> &error) {
	...
	struct dirent *file;
	while ((errno = 0, file = readdir(dir)) != NULL) {
		std::string full_path = dname + "/" + file->d_name;
		...
		if (S_ISDIR(statbuf.st_mode)) {
			if (!isdot(file->d_name) && RECUR) {
				#pragma omp task shared(error)
				walkDirPar(full_path, comp, error);
			}
		} else {
			char *name_cp = strdup(full_path.c_str());
			if (discardIt(name_cp, comp)) {
				...
				#pragma omp task shared(error)
				if (!doWork(name_cp, statbuf.st_size, comp))
					error.store(true, std::memory_order_relaxed);
			}
		}
	}
